// theme colors
export const themeColors = {
  terracotta: "#FF7A5C", // terracotta
  palemint: "#A8E6CF", // pale mint
  misspiggy: "#FFB2EF", // misspiggy
  skyblue: "#87CEEB", // sky blue
  lightgreen: "#90EE90", // light green
  lightsalmon: "#FFA07A", // light salmon
  pink: "#FFC0CB", // pink
  tropicsblue: "#A7DBD8", // tropics blue
  mintgreen: "#BAFCA2", // mint green
  yellow: "#FFDB58", // yellow
};

// proficiency scale
export const proficiencyScale = {
  novice_: "#FFA8A9", // novice
  adv_begin_: "#FFDB58", // advanced beginner
  competent_: "#86CCA0", // competent
  proficient_: "#1D6EE5", // proficient
  expert_: "#111616", // expert
};

// metals
export const themeMetallics = {
  darkteal: "#2B4F4F", // dark teal
  industrialgray: "#3C3C3C", // industrial gray
  texturedbrass: "#C0A568", // textured brass
  deepcopper: "#8D5524", // deep copper
  goldenyellow: "#D4AF37", // golden yellow
  platinum: "#E5E4E2", // platinum
  pewter: "#899499", // pewter
  ashes: "#B2BEB5", // ashen gray
  statueofliberty: "#A1CFB5", // statue of liberty
  cobalt: "#0047AB", // cobalt
};

// inks
export const themeInks = {
  agedparchment: "#F8ECD5", // aged parchment
  antiqueblack: "#111111", // antique black
  sepiabrown: "#534F32", // sepia brown
  indigo: "#5B5097", // indigo
  midnight: "#36454F", // midnight blue
  neonblue: "#1F51FF", // neon blue
  brightblue: "#00A1E4", // bright blue
  cobalt: "#336CBC", // cobalt light
};
